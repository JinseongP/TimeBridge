...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.cond_embed = cond
        if cond:
            self.emb = Conv_MLP(n_feat, int(n_embd/2), resid_pdrop=resid_pdrop)
            self.cond_emb = Conv_MLP(n_feat, int(n_embd/2), resid_pdrop=resid_pdrop)
        else:
    ...
    def forward(self, input, t, padding_masks=None, cond=None):
        emb = self.emb(input)
        if self.cond_embed:
