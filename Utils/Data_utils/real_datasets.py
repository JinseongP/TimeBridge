...
class CustomDataset(Dataset):
    def __init__(
        ...
        assert period in ['train', 'test', 'train-cond'], 'period must be train or test.'
        if period == 'train':
        ...
        self.samples = train if 'train' in period else inference
        if period == 'test' or period == 'train-cond':
    ...
    def __getitem__(self, ind):
        if self.period == 'test' or self.period == 'train-cond':
            x = self.samples[ind, :, :]  # (seq_length, feat_dim) array
            m = self.masking[ind, :, :]  # (seq_length, feat_dim) boolean array
            return torch.from_numpy(x).float(), torch.from_numpy(m), ind
        x = self.samples[ind, :, :]  # (seq_length, feat_dim) array
